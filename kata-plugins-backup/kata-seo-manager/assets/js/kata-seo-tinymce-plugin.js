  ...
  "url": "https://example.com/author/nguyen-van-a"
        ...
        const showPreviewAndInsert = (key) => {
            ...
            window['kataUpdatePreview_' + key] = function() {
                ...
                const checkedBoxes = document.querySelectorAll('#show_content_' + key + ' .kata-content-checkbox:checked');
                const checkedFields = [];
                ...
                if (checkedFields.length > 0) {
                    checkedFields.forEach((field) => {
                        ...
                        const hidePattern = new RegExp('hide_content_' + field + '="true"\\s*', 'g');
                ...
                buttons: [
                    {
                        ...
                        onclick: function() {
                            // Get base shortcode
                            let shortcode = templates[key].shortcode;
                            
                            // Get checked content fields
                            const checkedFields = [];
                            const checkboxes = document.querySelectorAll('#show_content_' + key + ' .kata-content-checkbox:checked');
        ...
        // Đóng dropdown menus khi format được apply (heading, bold, italic, etc)
        editor.on('NodeChange', function(e) {
            // Tìm ONLY dropdown menus (không bao gồm mce-panel/mce-container)
            setTimeout(function() {
                try {
                    // CHỈ đóng floatpanel và menu dropdowns, KHÔNG đóng mce-panel/mce-container
                    var dropdownMenus = jQuery('.mce-floatpanel:visible, .mce-menu:visible').not('.mce-tinymce, .mce-container, .mce-panel');
                    ...
                    if (dropdownMenus.length > 0) {
                        ...
                        if (selectedFormat && (
                            ...
                            // CHỈ đóng dropdown menus, KHÔNG đóng editor panels
                            dropdownMenus.hide();
                            
                            // KHÔNG gọi windowManager.close() vì nó sẽ đóng cả dialog
                        }
                    }
                } catch(err) {
                    // Fail silently để không ảnh hưởng editor
                    console.warn('KATA SEO Manager: Could not close dropdown menus', err);
                }
            }, 50); // Delay nhỏ để format được apply trước
        });
        
        // Đóng dropdown menus khi click vào editor content
        editor.on('click', function(e) {
            setTimeout(function() {
                try {
                    var target = e.target;
                    
                    // Kiểm tra xem có click vào toolbar/menubar/dropdown KHÔNG
                    var isToolbarClick = jQuery(target).closest('.mce-toolbar, .mce-menubar, .mce-floatpanel, .mce-menu').length > 0;
                    
                    // Kiểm tra xem có click vào TinyMCE container/panel (editor body) KHÔNG
                    var isEditorPanelClick = jQuery(target).closest('.mce-tinymce, .mce-container, .mce-panel').length > 0;
                    
                    // CHỈ đóng dropdown menus nếu click vào editor content (KHÔNG phải toolbar/dropdown)
                    // NHƯNG cho phép click vào mce-panel/mce-container mà KHÔNG đóng chúng
                    if (!isToolbarClick) {
                        // CHỈ đóng floatpanel/menu dropdowns, GIỮ NGUYÊN mce-panel/mce-container
                        jQuery('.mce-floatpanel:visible, .mce-menu:visible').not('.mce-tinymce, .mce-container, .mce-panel').hide();
                    }
                } catch(err) {
        ...
        const renderEditForm = (parsed, isContainer) => {
            ...
            Object.keys(categories).forEach((category) => {
                ...
                categoryAttrs.forEach((key) => {
                    ...
                    // Textarea for long text
                        ...
                        html += `<textarea id="attr_${key}" rows="3" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:13px;">${value}</textarea>`;
                    }
                    // Regular input
                    else {
                        html += `<input type="text" id="attr_${key}" value="${value}" style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:13px;">`;
                    }
            ...
            if (!hasAttributes) {
                if (isContainer) {
                    ...
                    html += `<p style="margin:0 0 20px 0;color:#4a148c;font-size:16px;line-height:1.6;"><strong>[${parsed.type}]</strong> is a container shortcode that wraps child items.</p>`;
