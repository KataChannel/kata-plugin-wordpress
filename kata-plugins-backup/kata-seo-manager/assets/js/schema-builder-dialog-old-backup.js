...
(function(window, tinymce) {
    ...
    // Constants
    const DIALOG_WIDTH = Math.min(window.innerWidth * 0.9, 900);
    const DIALOG_HEIGHT = Math.min(window.innerHeight * 0.85, 650);
    const CHECKBOX_COLUMNS = 2; // For better mobile experience
    ...
    const KataSchemaBuilder = {
            ...
            setTimeout(function() {
                try {
                    ...
                    if (dialog) {
                        ...
                        var checkboxContainers = dialog.querySelectorAll('.mce-container-body .mce-checkbox');
                        self._wrapCheckboxesInGrid(checkboxContainers);
        ...
        _wrapCheckboxesInGrid: function(checkboxes) {
            ...
            for (var i = 0; i < checkboxArray.length; i++) {
                ...
                if (prevSibling && prevSibling.classList.contains('mce-label') && 
                    (prevSibling.textContent.indexOf('SCHEMA') > -1 || 
                     prevSibling.textContent.indexOf('CONTENT') > -1)) {
        ...
        _createQuickActions: function() {
            return {
                type: 'label',
                text: '⚡ Mẹo: Click vào tiêu đề section để thu gọn/mở rộng',
    ...
    window.KataSchemaBuilder = KataSchemaBuilder;
        ...
        _buildDialogBody: function(schemaType, config) {
            ...
            // Schema properties section
            this._addSection(items, '🔍 SCHEMA PROPERTIES', this._buildSchemaFields(schemaType, config), 
                'Chọn thuộc tính xuất hiện trong JSON-LD schema (mặc định: tất cả)');

            // Content display section
            this._addSection(items, '🎨 CONTENT DISPLAY', this._buildContentFields(schemaType, config),
                'Chọn phần tử hiển thị trên frontend (mặc định: tất cả)');
        ...
        _createHeaderLabel: function(config) {
            return {
                type: 'label',
                text: '� Điền thông tin và chọn thuộc tính cần hiển thị. Các trường có dấu (*) là bắt buộc.'
            };
        },
        ...
        _createFooterLabel: function() {
            return {
                type: 'label',
                text: '✨ Tip: Bỏ chọn checkbox sẽ thêm thuộc tính hide_* vào shortcode'
            };
        },
        ...
        _buildBasicFields: function(config) {
            const fields = [];
            const entries = Object.entries(config.baseAttrs);

            entries.forEach(function([key, attr]) {
                fields.push({
                    type: 'textbox',
                    name: key,
                    label: attr.label + (attr.required ? ' *' : ''),
                    ...
                    placeholder: attr.placeholder || attr.label,
                    multiline: attr.multiline || false,
                    minHeight: attr.multiline ? 80 : undefined
                });
            });
         ...
         */
        _addBasicAttributes: function(parts, config, data) {
            Object.keys(config.baseAttrs).forEach(function(key) {
                const value = data[key];
                if (value && String(value).trim() !== '') {
                    parts.push(key + '="' + KataSchemaBuilder._escapeAttr(value) + '"');
                }
            });
        },
         ...
         */
        _addSchemaAttributes: function(parts, config, data) {
            config.schemaFields.forEach(function(field) {
         ...
         */
        _addContentAttributes: function(parts, config, data) {
            config.contentFields.forEach(function(field) {
